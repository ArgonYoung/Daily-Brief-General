...
async function main() {
  ...
  async run({ inputs, sdk }) {
    ...
    try {
      ...
      const prompt = "你是一个专业的老练射击游戏玩家与无畏契约（VALORANT）皮肤研究专家。\\n" +
        "今天商店里刷出了以下这几款武器皮肤，请结合当前玩家社区的互联网趋势、讨论热度、皮肤特效声效评价与性价比进行综合评估：\\n" +
        itemsInfo.join("\\n") + "\\n" +
        "评价规范：\\n" +
        "1. 客观专业，结合玩家公认的皮肤梯队（如 T0/T1/T2 评级）和手感反馈，避免盲目刻薄或强行网感，以实用购买指导为主。\\n" +
        "2. 字数控制在 250 字以内，段落清晰，每一把武器都单独换行写出评价。\\n" +
        "3. 直接输出段落内容，不要包含任何多余的引言或代码框。";
        
      const aiReview = await sdk.llm.complete(prompt);
      const quotedReview = aiReview.split(/\\r?\\n/).map(l => l.trim()).filter(l => l).map(l => "> " + l).join("\\n");
      content += "\\n**商店评语：**\\n" + quotedReview;
